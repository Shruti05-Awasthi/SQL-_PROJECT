...
-- Question1. find out the top  3 product id based on the quantity sold?
select product_id from order_lines group by product_id order by sum(quantity) desc limit 3;
-- Question2 get the order date the product id , the product name , and the order id  which has been sold in the multiples of 2?
SELECT 
    o.order_date,
    ol.product_id,
    p.product_name,
...
-- Question3 get the brand name, the order value and the product name for each product sold?
SELECT 
    b.brand,
    o.order_value,
    p.product_name
...
INNER JOIN orders o ON ol.order_id = o.id
INNER JOIN products p ON ol.product_id = p.product_id
INNER JOIN brands b ON ol.product_id = b.product_id;
-- Question4  get the average order value for each order which has been placed after half and hour?
SELECT 
    AVG(order_value) AS average_order_value
FROM orders
WHERE MINUTE(order_date) >= 30;
-- Question5 get the order id , the number of quantity sold , the month  and the total price for each brand ?
SELECT 
    ol.order_id,
...
INNER JOIN products p ON ol.product_id = p.product_id
INNER JOIN brands b ON ol.product_id = b.product_id
GROUP BY ol.order_id, MONTH(o.order_date), b.brand
ORDER BY ol.order_id, b.brand;
